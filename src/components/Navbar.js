...
export default function Navbar() {
  ...
  return (
    <header className="relative">
      <div className={`bg-green-800 text-white text-sm p-2 flex end items-center ${isScrolled ? 'hidden' : ''}`}>
        <span>Phone: 303-358-8168</span>
        <span>Email: admin@gvsco.net</span>
      </div>
      <nav className={`bg-white text-black p-4 sticky top-0 z-50 shadow-md ${isScrolled ? 'shadow-md' : ''}`}>
        
      <div className="container mx-auto flex justify-between items-center text-sm sm:text-base md:text-lg lg:text-xl xl:text-2xl">          
          {/* Logo */}
          <div className="flex-shrink-0 flex left">
            <Image 
